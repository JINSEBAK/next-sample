...
const PlaceMapPage = () => {
  //
  const markerImageSrc = "/images/inc/icon_map_current_location.svg";
  ...
  useLayoutEffect(() => {
    const lists = [];
    [...Array(126)].map((item, index) => {
      lists.push({
        id: `f${index}`,
        imgUrl: `/images/test${index % 9}.jpeg`,
        position: { lat: positions[index].lat, lng: positions[index].lng },
      });
    });
    setMarkers(lists);
  }, []);
  ...
  return (
    <MainContainer>
      <Script
        src={`//dapi.kakao.com/v2/maps/sdk.js?appkey=3dae0e1c3bcd367cdf273b9533ba7cce&autoload=false&libraries=services,clusterer`}
      ...
      <Contents className="ssg_search_place">
        ...
        >
          ...
          >
            {/* 사용자의 현재 위치 marker */}
            <MapMarker
              position={{ lat: 37.4783066104075, lng: 126.887377235326 }}
              image={{ src: markerImageSrc, size: imageSize }}
            />
            <MarkerClusterer
              averageCenter={true}
              minLevel={5}
              disableClickZoom={true} // 마커 클릭시 지도 확대 비활성화 여부
                  ...
                  return (
                    <Fragment key={index}>
                      <CustomOverlayMap position={item.position}>
                        <div className="group_pin">
                          <button type="button" className="btn_img_pin">
                            <img
                              src={item.imgUrl}
                              alt="이 구역 가장 인기많은 피드"
                            />
